...
public class VigenereTest {
    @Test
    public void doTests() {
        assertEquals("BBB", Vigenere.encode("AAA", "AAA")); // OK \\
        assertEquals("ZZZ", Vigenere.encode("YYY", "AAA")); // OK \\
        assertEquals("BCD", Vigenere.encode("ABC", "AA")); // OK \\
        assertEquals("CPM VFE", Vigenere.encode("BEN FET", "AKYP")); // OK \\
        assertEquals("YQ USNCT WK WYWEWMF", Vigenere.encode("El Camió és vermell", "terra"));
        assertEquals("DESBC", Vigenere.encode("CDRAB", "AAA")); // OK \\
        assertEquals("VOUMYSLVQVY", Vigenere.encode("JFRHDGCSLAM", "LICEU")); // OK \\
        assertEquals("AAA", Vigenere.encode("ZZZ", "AAA")); // OK \\
        assertEquals("NWGU US EPA EUM", Vigenere.encode("AVUI FA BON DIA", "MALLORCA")); // OK \\
